...
def show():
    ...
    try:
        df = pd.read_csv("./data/supermarket_sales.csv", sep=",")  # Ajuste o caminho se necessário
    except FileNotFoundError as e:
        st.error("Arquivo não encontrado. Verifique se 'supermarket_sales.csv' está no diretório correto.")
    ...
    df["Classificação"] = df["Classificação"].str.replace(',', '.').astype(
        float)  # Substituir vírgulas por pontos e converter para float

    # Tratar a coluna 'Total' para garantir que seja numérica
    df["Total"] = df["Total"].str.replace(',', '.').astype(
        float)  # Substituir vírgulas por pontos e converter para float
    ...
    fig_prod = px.bar(df_filtered, x="Linha de produtos", y="Total", color="Cidade",
                      title="Faturamento por produto", orientation="h")
    ...
    if total_faturamento > 0:  # Verifica se o total não é zero para evitar divisão por zero
        ...
        city_total["Porcentagem"] = 0  # Se o total for zero, define a porcentagem como zero

    fig_city = px.bar(city_total, x="Cidade", y="Porcentagem", title="Faturamento por filial (%)")
    col3.plotly_chart(fig_city, use_container_width=True)

    # Receita por tipo de pagamento (corrigido para usar 'Pagamento')
    fig_kind = px.pie(df_filtered, values="Total", names="Pagamento", title="Faturamento por pagamento")
    col4.plotly_chart(fig_kind, use_container_width=True)

    # Avaliação média por cidade (alterado para "Classificação")
    city_rating = df_filtered.groupby("Cidade")[["Classificação"]].mean().reset_index()  # Usar "Classificação"
    fig_rating = px.bar(city_rating, x="Cidade", y="Classificação", title="Avaliação")  # Alterado para "Avaliação"
